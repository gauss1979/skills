...
认证信息和站点配置统一存储在 `$AMBER_DIR/` 目录下（由调用方通过 exec `env` 参数注入）：

| 文件 | 内容 |
|------|------|
| `$AMBER_DIR/token` | Bearer Token |
| `$AMBER_DIR/config.json` | 站点配置（站点ID、NMI、电网等） |

> 调用 agent 必须通过 `env: {"AMBER_DIR": "<agent_workspace>/.amber"}` 传入路径，脚本仅读取该变量，不做路径推断。各 agent 的 workspace 路径：`${agents.<agentId>.workspace}`。
...
> site_id 可省略，首次登录后自动从 `$AMBER_DIR/config.json` 读取。
...
- Token 认证信息仅存储在 `$AMBER_DIR/token`，**不写入记忆或技能文件**
