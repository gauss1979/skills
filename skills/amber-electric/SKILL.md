...
| 文件 | 内容 |
|------|------|
| `~/.amber/credentials.json` | 用户名 + 密码（OAuth 2.0） |
| `~/.amber/config.json` | 站点配置（站点ID、NMI、电网等） |

## 认证配置（用户名+密码）

Amber API 使用 OAuth 2.0 Password Grant 认证。首次使用需提供用户名和密码：
...
认证成功后会保存凭证到 `~/.amber/credentials.json`，后续命令无需再输入用户名和密码。
...
```bash
# 首次认证
amber.py login myemail@example.com mypassword123
...
- 认证信息（用户名+密码）存储在 `~/.amber/credentials.json`（600权限），不写入记忆
- 凭证失效时自动提示，重新登录即可：`amber.py login <用户名> <密码>`
