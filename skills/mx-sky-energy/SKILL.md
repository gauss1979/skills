...
所有收益、功率、充放电相关命令执行后**必须自动生成图表**，并发送给用户。
...
1. **生成图表**：调用 `chart` 技能（`make_chart.py`）或直接生成 PNG 保存到输出目录
2. **输出位置**：`~/.openclaw/workspace/memory/chart/output/`
3. **发送给用户**：使用 `message` 工具发送图片 + 文字摘要到飞书
   - `channel=feishu`
   - `target` = 当前会话用户 ID
   - `media` = 图表文件路径
   - `caption` = 文字摘要（含关键数据）
...
- **必须发送图表**：所有收益查询（`earnings-week`、`earnings-year`、`report`）必须附带图表
- **先文字后图表**：回复文字摘要后再发送图表，不要只发图表
- **caption 要点**：说明站点、周期、关键数字（总收益、最高/最低日）
- **webchat 用户**：图表无法内嵌时，在文字中说明图表已生成，用户可切换到飞书查看完整版
...
所有命令执行后必须先回复文字摘要，格式如下：

**列表查询**：
```
📍 [站点名称]  [站点ID]
   位置: [城市], [省份]
   ⚡ 光伏: X kW  | 🔋 SOC: X%  | 🔌 电池: X kW
   ⏰ 更新: YYYY-MM-DD HH:mm
```
...
收益相关命令执行后，**必须**使用 `message` 工具发送图表：

```python
message(
    action="send",
    channel="feishu",
    target="user:<sender_id>",  # 当前会话用户 ID
    media="<图表文件路径>",
    caption="<文字摘要>"
)
```

**示例**：查询周收益后，同步发送柱状图，caption 包含本周总收益和趋势说明。
